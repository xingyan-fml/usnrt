...
class variance_nn(model_nn):
    def __init__(self, in_dim, hiddens):
        super(variance_nn, self).__init__(in_dim, hiddens)
        self.positive = nn.Softplus()
    
    def forward(self, X):
        X = super(variance_nn, self).forward(X)
...
def fit_variance_nn(data, var_cont, var_cate, y, Y_hat, hidden_dims, num_epochs, batch_size, lr, train_prop, set_seed):
...
if __name__ == "__main__":
    ...
    Y_hat = data['y'].values.reshape(-1) - residual
    variance_nn, var_hat = fit_variance_nn(data, data.columns[:-1], [], 'y', Y_hat, hidden_dims,
                num_epochs=100, batch_size=64, lr=0.01, train_prop=0.8, set_seed=10)
    ...
    compare['pred_std'] = np.sqrt(var_hat)
    print(compare)
