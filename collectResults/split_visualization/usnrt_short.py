...
def grow_tree(data, var_cont, var_cate, y, split_dims, leaf_dims, p_value = 0.05,
    ...
    if len(data) < 2*Nmin:
        root.model, root.residual, root.sigma2 = fit_nn(data_onehot, var_cont, var_cate, y,
                    leaf_dims, num_epochs, batch_size, lr, train_prop, set_seed + index[0])
        Y_hat = data_onehot[y].values.reshape(-1) - root.residual
        root.model_uncertainty, root.var_hat = fit_variance_nn(data_onehot, var_cont, var_cate, y,
                    Y_hat, leaf_dims, num_epochs, batch_size, lr, train_prop, set_seed + index[0])
        return root
