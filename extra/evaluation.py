...
#==============================================================================
    ...
    for t in Trees:
        for d in Depths:
            for f in Features:
                extra = ExtraTreesRegressor(n_estimators = t, max_depth = d, max_features = f, 
                                            n_jobs = 16, random_state = set_seed)
                model_extra = extra.fit(train_data_onehot, train_data[y])
                mu_extra, std_extra = model_extra.predict(valid_data_onehot, return_std = True)
                valid_loss = NLLloss(valid_data[y], mu_extra, std_extra ** 2)
                if valid_loss < best_valid:
                    best_valid = valid_loss
                    best_model = copy.deepcopy(model_extra)

    mu_extra, std_extra = best_model.predict(test_data_onehot, return_std = True)
    var_extra = std_extra ** 2
    quantiles = gen_quantiles(mu_extra, var_extra, tau)
    ...
    with open(data_dir + method_name + ".csv", 'a') as file:
        file.write("{},{},{}\n".format(set_seed, cal_ECE(test_data[y], quantiles, tau), rmse(mu_extra, test_data[y])))
    
    if not os.path.isdir(data_dir + method_name + "/"):
        os.mkdir(data_dir + method_name + "/")
    results = pd.DataFrame([mu_extra.tolist(), var_extra.tolist(), test_data[y].tolist()]).T
